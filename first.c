...
int WRITEcheckprefetch(unsigned long long tag,unsigned long long int setIndex,int assoc){
	int leastUsed;
	int i=0;
	while(assoc>i){
...
int READcheckprefetch(unsigned long long tag,unsigned long long int setIndex,int assoc){
	...
	while(i<assoc){
		if(cache[setIndex][i].filled!=0){
			...
			if((i+1)==assoc){
	        	...
	        	WRITEcheckprefetch(pretag, preSetIndex,assoc);
	        	return 0;
			}
		}else{
			miss=miss+1;
	...
	while(i<assoc){
		if(cache[setIndex][i].filled!=0){
			...
			if((i+1)==assoc){
	        	...
	        	READcheckprefetch(pretag,preSetIndex,assoc);
	        	return 0;
			}
		}else{
		miss=miss+1;
